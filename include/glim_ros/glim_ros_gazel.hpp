...
class GlimROSGazel {
public:
  GlimROSGazel(ros::NodeHandle& nh);
  ~GlimROSGazel();
...
//  void insert_image(const double stamp, const cv::Mat& image);
  ...
  void insert_gkv(double stamp,const gtsam::Pose3& pose, const Eigen::Vector3d& v, const Eigen::Vector3d& w);
  void insert_vd(double stamp,double w, double v);
  void insert_loc(double stamp,const gtsam::Pose3& pose);
...
private:
  ...
  std::unique_ptr<glim::AsyncNavigation> gazel_nav_estimation;
