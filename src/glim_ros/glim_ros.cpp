...
GlimROS::GlimROS(ros::NodeHandle& nh) : tf_buffer(ros::Duration(100.)), tf_listener(tf_buffer) {
  ...
  // Sub mapping
  if (config_ros.param<bool>("glim_ros", "enable_sub_mapping", false)) { 
  ...
  // Global mapping
  if (config_ros.param<bool>("glim_ros", "enable_global_mapping", false)) { 
...
//  preprocessed->raw_points = raw_points;
  ...
  while (odometry_estimation->workload() > 10) {
    ...
    std::cout << "before insert_frame: " << duration<double, std::milli>( t1- high_resolution_clock::now()).count() << std::endl;
